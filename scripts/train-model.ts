...
const MODEL_NAME    = args["model"]           ?? process.env["FINE_TUNE_MODEL"] ?? "Qwen2.5-0.5B-Instruct";
const PROVIDER_ADDR = args["provider"]        ?? process.env["PROVIDER_ADDR"]   ?? "";
const OUTPUT_DIR    = args["output"]          ?? process.env["OUTPUT_DIR"]      ?? "./output/token-classifier";
...
function writeLastFineTuneTaskIdMarker(outputPath: string, taskId: string): void {
  try {
    fs.writeFileSync(
...
const TOKENS: TokenEntry[] = [
  // ── L1 ──────────────────────────────────────────────────────────────────
  { symbol: "ETH",     name: "Ethereum",           category: "L1",   address: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE", explanation: "Native token of Ethereum, the leading smart-contract Layer 1 blockchain. The placeholder address is a routing convention for native ETH (not an ERC-20 contract); use WETH for the canonical wrapped ERC-20." },
  { symbol: "WETH",    name: "Wrapped Ether",      category: "L1",   address: "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", explanation: "ERC-20 wrapped ETH on Ethereum mainnet — 1:1 backed native ETH in the WETH9 contract for DeFi and smart-contract compatibility." },
  { symbol: "BTC",     name: "Bitcoin",             category: "L1",   address: "0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599", explanation: "Native token of Bitcoin, the original proof-of-work Layer 1 blockchain. On Ethereum mainnet the canonical 1:1 backed representation is Wrapped BTC (WBTC) at this ERC-20 address." },
  { symbol: "SOL",     name: "Solana",              category: "L1",   explanation: "Native token of Solana, a high-throughput proof-of-stake Layer 1 blockchain." },
  { symbol: "AVAX",    name: "Avalanche",           category: "L1",   explanation: "Native token of Avalanche, a fast Layer 1 with subnet architecture." },
  { symbol: "BNB",     name: "BNB Chain",           category: "L1",   address: "0xB8c77482e45F1F44dE1745F52C74426C631bDD52", explanation: "Native token of BNB Chain, Binance's EVM-compatible Layer 1 blockchain." },
  { symbol: "ADA",     name: "Cardano",             category: "L1",   explanation: "Native token of Cardano, a proof-of-stake Layer 1 with formal verification." },
  { symbol: "DOT",     name: "Polkadot",            category: "L1",   explanation: "Native token of Polkadot, a Layer 1 parachain relay network." },
  { symbol: "ATOM",    name: "Cosmos Hub",          category: "L1",   explanation: "Native token of Cosmos Hub, a Layer 1 blockchain for the interchain ecosystem." },
  { symbol: "NEAR",    name: "NEAR Protocol",       category: "L1",   explanation: "Native token of NEAR Protocol, a sharded proof-of-stake Layer 1." },
  { symbol: "SUI",     name: "Sui",                 category: "L1",   explanation: "Native token of Sui, a Move-based high-performance Layer 1." },
  { symbol: "APT",     name: "Aptos",               category: "L1",   explanation: "Native token of Aptos, a Move-based Layer 1 blockchain by ex-Meta engineers." },
  { symbol: "SEI",     name: "Sei",                 category: "L1",   explanation: "Native token of Sei, a Layer 1 optimised for financial applications." },
  { symbol: "INJ",     name: "Injective",           category: "L1",   address: "0xe28b3B32B6c345A34Ff64674606124Dd5Aceca30", explanation: "Native token of Injective, a Layer 1 built for DeFi and derivatives." },
  { symbol: "TRX",     name: "Tron",                category: "L1",   explanation: "Native token of Tron, a Layer 1 blockchain for digital content and stablecoins." },
  { symbol: "TON",     name: "The Open Network",    category: "L1",   explanation: "Native token of TON, a Layer 1 originally developed by Telegram." },
  { symbol: "ZG",      name: "0G Network",          category: "L1",   explanation: "Native token of 0G Network, a Layer 1 blockchain for decentralised AI infrastructure." },
  ...
  // ── L2 ──────────────────────────────────────────────────────────────────
  { symbol: "ARB",     name: "Arbitrum",            category: "L2",   address: "0xB50721BCf8d664c30412Cfbc6cf7a15145234ad1", explanation: "Governance token of Arbitrum, a leading Ethereum Layer 2 optimistic rollup." },
  { symbol: "OP",      name: "Optimism",            category: "L2",   explanation: "Governance token of Optimism, an Ethereum Layer 2 optimistic rollup. OP is an ERC-20 native to Optimism (0x4200000000000000000000000000000000000042); Optimism's official token metadata lists no separate bridged OP contract on Ethereum L1." },
  { symbol: "MATIC",   name: "Polygon (MATIC)",     category: "L2",   address: "0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0", explanation: "Legacy Polygon ERC-20 on Ethereum (original MATIC ticker). Polygon's successor ecosystem token on Ethereum is POL; MATIC remains valid for this historic contract." },
  { symbol: "POL",     name: "Polygon (POL)",       category: "L2",   address: "0x455e53CBB86018Ac2B8092FdCd39d8444aFFC3F6", explanation: "Polygon Ecosystem Token (POL) on Ethereum — the MATIC successor used for Polygon PoS staking, gas, and emissions per Polygon governance." },
  { symbol: "IMX",     name: "Immutable X",         category: "L2",   address: "0xF57e7e7C23978C3cAEC3C3548E3D615c346e79fF", explanation: "Native token of Immutable X, an Ethereum Layer 2 for NFTs using ZK proofs." },
  { symbol: "METIS",   name: "Metis",               category: "L2",   address: "0x9E32b13ce7f2E80A01932B42553652E053D6ed8e", explanation: "Native token of Metis, an Ethereum Layer 2 optimistic rollup." },
  { symbol: "BOBA",    name: "Boba Network",        category: "L2",   address: "0x42bBFa2e77757C645eeaAd1655E0911a7553Efbc", explanation: "Governance token of Boba Network, an Ethereum Layer 2 optimistic rollup." },
  { symbol: "STRK",    name: "Starknet",            category: "L2",   address: "0xCa14007Eff0dB1f8135f4C25B34De49AB0d42766", explanation: "Native token of Starknet, an Ethereum Layer 2 ZK-STARK rollup. The STRK ERC-20 is deployed on Ethereum mainnet (governance, bridging, and CEX custody)." },
  { symbol: "MANTA",   name: "Manta Pacific",       category: "L2",   address: "0xa7ba16B12A5b068CB0Af480db33Ee68D13819AFA", explanation: "Native token of Manta Pacific, a Layer 2 ZK network for modular DeFi. MANTA ERC-20 on Ethereum mainnet for liquidity and bridging." },
  { symbol: "BLAST",   name: "Blast",               category: "L2",   explanation: "Native token of Blast, an Ethereum Layer 2 with built-in native yield. BLAST trades primarily on Blast L2; no single canonical BLAST ERC-20 on Ethereum L1 is listed in this dataset." },
  { symbol: "SCROLL",  name: "Scroll",              category: "L2",   explanation: "Native token of Scroll (SCR), an Ethereum Layer 2 ZK-EVM rollup. SCR is issued on Scroll; bridge-wrapped L1 addresses vary by bridge — omitted here to avoid ambiguous contracts." },
  { symbol: "ZKS",     name: "zkSync",              category: "L2",   address: "0x66A5cFB2e9c529f14FE6364Ad1075dF3a649C0A5", explanation: "Governance token of ZKsync (ticker ZK on-chain), an Ethereum Layer 2 ZK-rollup. This is the official ZK ERC-20 on Ethereum mainnet." },
  // ── DeFi ────────────────────────────────────────────────────────────────
  { symbol: "UNI",     name: "Uniswap",             category: "DeFi", address: "0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984", explanation: "Governance token of Uniswap, the largest decentralised exchange (DEX) on Ethereum." },
  { symbol: "AAVE",    name: "Aave",                category: "DeFi", address: "0x7Fc66500c84A76Ad7e9c93437bFc5Ac33E2DDaE9", explanation: "Governance token of Aave, a leading decentralised lending and borrowing protocol." },
  { symbol: "COMP",    name: "Compound",            category: "DeFi", address: "0xc00e94Cb662C3520282E6f5717214004A7f26888", explanation: "Governance token of Compound, a decentralised money-market protocol." },
  { symbol: "CRV",     name: "Curve Finance",       category: "DeFi", address: "0xD533a949740bb3306d119CC777fa900bA034cd52", explanation: "Governance token of Curve Finance, a DEX specialising in stable-asset swaps." },
  { symbol: "SNX",     name: "Synthetix",           category: "DeFi", address: "0xC011a73ee8576Fb46F5E1c5751cA3B9Fe0af2a6F", explanation: "Governance token of Synthetix, a decentralised synthetic-asset issuance protocol." },
  { symbol: "MKR",     name: "MakerDAO",            category: "DeFi", address: "0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2", explanation: "Governance token of MakerDAO, the protocol behind the DAI stablecoin." },
  { symbol: "BAL",     name: "Balancer",            category: "DeFi", address: "0xba100000625a3754423978a60c9317c58a424e3D", explanation: "Governance token of Balancer, a decentralised weighted AMM." },
  { symbol: "1INCH",   name: "1inch",               category: "DeFi", address: "0x111111111117dC0aa78b770fA6A738034120C302", explanation: "Governance token of 1inch, a DEX aggregator finding best swap routes." },
  { symbol: "SUSHI",   name: "SushiSwap",           category: "DeFi", address: "0x6B3595068778DD592e39A122f4f5a5cF09C90fE2", explanation: "Governance token of SushiSwap, a community-driven decentralised exchange." },
  { symbol: "YFI",     name: "Yearn Finance",       category: "DeFi", address: "0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e", explanation: "Governance token of Yearn Finance, a yield-optimisation DeFi aggregator." },
  { symbol: "LDO",     name: "Lido",                category: "DeFi", address: "0x5A98FcBEA516Cf06857215779Fd812CA3beF1B32", explanation: "Governance token of Lido, the largest liquid staking DeFi protocol." },
  { symbol: "PENDLE",  name: "Pendle Finance",      category: "DeFi", address: "0x808507121B80c02388fAd14726482e061B8da827", explanation: "Governance token of Pendle, a DeFi yield-trading and splitting protocol." },
  { symbol: "GMX",     name: "GMX",                 category: "DeFi", address: "0xD28807D7eF028AF6728d12Ccd621b2242Da2a64f", explanation: "Governance and utility token of GMX, a decentralised perpetual exchange. Protocol activity is on Arbitrum; this is the GMX ERC-20 on Ethereum mainnet (e.g. bridged holdings)." },
  { symbol: "DYDX",    name: "dYdX",                category: "DeFi", address: "0x92D6C1e31e14520e676a687F0a93788B716BEff5", explanation: "Governance token of dYdX, a decentralised perpetual trading protocol." },
  { symbol: "JUP",     name: "Jupiter",             category: "DeFi", explanation: "Governance token of Jupiter, the leading DEX aggregator on Solana." },
  { symbol: "GNS",     name: "Gains Network",       category: "DeFi", explanation: "Governance token of Gains Network (gTrade), a decentralised leveraged-trading platform." },
  // ── RWA ─────────────────────────────────────────────────────────────────
  { symbol: "ONDO",    name: "Ondo Finance",        category: "RWA",  address: "0xfAbA6f8e4a5E8Ab82F62fe7C39859FA577269BE3", explanation: "Token of Ondo Finance, a protocol issuing tokenised US Treasuries and other real-world assets." },
  { symbol: "PAXG",    name: "PAX Gold",            category: "RWA",  address: "0x45804880De22913dAFE09f4980848ECE6EcbAf78", explanation: "Each PAXG token is backed by one troy ounce of physical gold — a real-world asset on-chain." },
  { symbol: "XAUT",    name: "Tether Gold",         category: "RWA",  address: "0x68749665ff8D2d112FA859aa851f5677e94f7fC6", explanation: "Each XAUT token is backed by physical gold held in a Swiss vault, a real-world asset." },
  { symbol: "CFG",     name: "Centrifuge",          category: "RWA",  address: "0xc221B7e65fFC80DE234bB6667abDd46593d34f0F", explanation: "Token of Centrifuge, a protocol tokenising invoices, mortgages and other real-world assets." },
  { symbol: "TRU",     name: "TrueFi",              category: "RWA",  address: "0x4C19596f5aAfF459fA38B0f7eD92F11AE6543784", explanation: "Governance token of TrueFi, an uncollateralised on-chain credit marketplace backed by real-world loans." },
  { symbol: "POLYX",   name: "Polymesh",            category: "RWA",  address: "0x9992eC3cF6A55b00978cdDF2b27BC6882d88D1eC", explanation: "Native token of Polymesh (POLYX) for regulated securities. On Ethereum, Polymath (POLY) at this address is locked via the official upgrade bridge to mint POLYX on Polymesh (1:1)." },
  { symbol: "MPLX",    name: "Maple Finance",       category: "RWA",  address: "0x33349B282065b0284d756F0577FB39c158F935e6", explanation: "Token of Maple Finance, an institutional lending protocol backed by real-world credit." },
  { symbol: "RIO",     name: "Realio Network",      category: "RWA",  address: "0x94a8b4EE5CD64C79D0Ee816f467EA73009f51aA0", explanation: "Token of Realio Network, a platform for tokenising real estate and financial instruments — Realio Network (RIO) ERC-20 on Ethereum mainnet." },
  // ── AI ───────────────────────────────────────────────────────────────────
  { symbol: "FET",     name: "Fetch.ai",            category: "AI",   address: "0xaea46A60368A7bD060eec7DF8CBa43b7EF41Ad85", explanation: "Token of Fetch.ai, a decentralised ML network powering autonomous AI agents." },
  { symbol: "OCEAN",   name: "Ocean Protocol",      category: "AI",   address: "0x967da4048cD07aB37855c090aAF366e4ce1b9F48", explanation: "Token of Ocean Protocol, a decentralised marketplace for AI data and services." },
  { symbol: "AGIX",    name: "SingularityNET",      category: "AI",   address: "0x5B7533812759B45C2B44C19e320ba2cD2681b542", explanation: "Token of SingularityNET, a decentralised marketplace for AI algorithms." },
  { symbol: "TAO",     name: "Bittensor",           category: "AI",   address: "0x77E06c9eCCf2E797fd462A92B6D7642EF85b0A44", explanation: "Token of Bittensor, a decentralised network that rewards AI model training. TAO is native to the Bittensor subnet; this address is Wrapped TAO (wTAO), the canonical ERC-20 on Ethereum mainnet." },
  { symbol: "AKT",     name: "Akash Network",       category: "AI",   address: "0xC727f87871ee12Bbcedd2973746D1Deb7529aaD6", explanation: "Token of Akash Network, a decentralised cloud-compute marketplace for AI workloads. This is the Akash Network (AKT) ERC-20 on Ethereum mainnet." },
  { symbol: "NMR",     name: "Numeraire",           category: "AI",   address: "0x1776e1F26f98b1A5dF9cD347953a26dd3Cb46671", explanation: "Token of Numerai, a hedge fund with a decentralised AI data-science tournament." },
  { symbol: "GRT",     name: "The Graph",           category: "AI",   address: "0xc944E90C64B2c07662A292be6244BDf05Cda44a7", explanation: "Token of The Graph, a decentralised indexing protocol enabling AI-style data queries on blockchains." },
  { symbol: "RNDR",    name: "Render Network",      category: "AI",   address: "0x6De037ef9aD2725EB40118Bb1702EBb27e4Aeb24", explanation: "Legacy Render (RNDR) ERC-20 on Ethereum; the network's primary token is now RENDER (SPL on Solana), but this contract remains the canonical Ethereum address for RNDR." },
  { symbol: "AIOZ",    name: "AIOZ Network",        category: "AI",   address: "0x626E8036dEB333b408Be468F951bdB42433cBF18", explanation: "Token of AIOZ Network, a decentralised AI and content-delivery network." },
  { symbol: "ALI",     name: "Alethea AI",          category: "AI",   address: "0x6B0b3a982b4634aC68dD83a4DBF02311cE324181", explanation: "Token of Alethea AI, a protocol for creating intelligent NFTs powered by AI." },
  { symbol: "VIRTUAL", name: "Virtuals Protocol",   category: "AI",   address: "0x44ff8620b8cA30902395A7bD3F2407e1A091BF73", explanation: "Token of Virtuals Protocol, a platform for deploying and co-owning AI agents — VIRTUAL ERC-20 on Ethereum mainnet (also deployed on Base per project docs)." },
  { symbol: "MASA",    name: "Masa Network",        category: "AI",   address: "0x944824290CC12F31ae18Ef51216A223Ba4063092", explanation: "Token of Masa Network, a decentralised data layer for AI training datasets — MASA ERC-20 on Ethereum mainnet (LayerZero OFT)." },
  // ── DeFi: oracle / LST / infra — common mis-categorisations ─────────────
  { symbol: "LINK",   name: "Chainlink",                    category: "DeFi", address: "0x514910771AF9Ca656af840dff83E8264EcF986CA", explanation: "LINK powers Chainlink's decentralised oracle network — DeFi infrastructure, NOT AI. Chainlink feeds price data on-chain; it does not run ML models or AI agents." },
  { symbol: "RPL",    name: "Rocket Pool",                  category: "DeFi", address: "0xD33526068D116cE69F19A9ee46F0bd304F21A51f", explanation: "RPL is the governance and insurance collateral token for Rocket Pool, a decentralised ETH liquid staking protocol — DeFi, not L2." },
  { symbol: "ENS",    name: "Ethereum Name Service",        category: "DeFi", address: "0xC18360217D8F7Ab5e7c516566761Ea12Ce7F9D72", explanation: "ENS governs the Ethereum Name Service on-chain registry — DeFi protocol governance, not L1 or L2." },
  { symbol: "BLUR",   name: "Blur NFT Marketplace",        category: "DeFi", address: "0x5283D291DBCF85356A21bA090E6db59121208b44", explanation: "BLUR is the governance token of the Blur NFT marketplace and lending protocol — DeFi, not AI." },
  { symbol: "ANKR",   name: "Ankr Network",                 category: "DeFi", address: "0x8290333ceF9e6D528dD5618Fb97a76f268f3EDD4", explanation: "ANKR powers Ankr's decentralised RPC and staking infrastructure — DeFi/infrastructure, NOT an AI protocol. General compute provision ≠ AI/ML models." },
  { symbol: "API3",   name: "API3",                         category: "DeFi", address: "0x0b38210ea11411557c13457D4dA7dC6ea731B88a", explanation: "API3 governs the API3 DAO, a decentralised first-party API oracle protocol — DeFi data-feed infrastructure, not AI." },
  { symbol: "BAND",   name: "Band Protocol",                category: "DeFi", address: "0xBA11D00c5f74255f56a5E366F4F77f5A186d7f55", explanation: "BAND powers Band Protocol's cross-chain oracle data feeds — DeFi infrastructure, not AI." },
  { symbol: "SHIB",   name: "Shiba Inu",                    category: "DeFi", address: "0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE", explanation: "SHIB is a DeFi meme token on Ethereum with its own ShibaSwap DEX ecosystem — DeFi, not AI or L1." },
  { symbol: "PEPE",   name: "Pepe",                         category: "DeFi", address: "0x6982508145454Ce325dDbE47a25d4ec3d2311933", explanation: "PEPE is a DeFi meme token on Ethereum mainnet — DeFi, not AI, L1, or L2." },
  // ── DeFi: liquid staking tokens (LSTs) — NOT L1 ──────────────────────────
  { symbol: "stETH",  name: "Lido Staked ETH",              category: "DeFi", address: "0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84", explanation: "stETH is Lido's liquid staking receipt token (DeFi), NOT L1. The underlying asset is ETH (L1) but stETH is a DeFi protocol derivative." },
  { symbol: "wstETH", name: "Lido Wrapped stETH",           category: "DeFi", address: "0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0", explanation: "wstETH is the wrapped non-rebasing form of Lido stETH — DeFi, not L1. It wraps staked ETH exposure into a DeFi ERC-20 for composability." },
  { symbol: "cbETH",  name: "Coinbase Wrapped Staked ETH",  category: "DeFi", address: "0xBe9895146f7AF43049ca1c1AE358B0541Ea49704", explanation: "cbETH is Coinbase's liquid staking ETH derivative — DeFi, not L1. It wraps staked ETH into an ERC-20 for DeFi use; the underlying ETH is L1 but cbETH is not." },
  { symbol: "rETH",   name: "Rocket Pool ETH",              category: "DeFi", address: "0xae78736Cd615f374D3085123A210448E74Fc6393", explanation: "rETH is Rocket Pool's ETH liquid staking receipt token — DeFi, not L1." },
  // ── L1: streaming / storage blockchains — NOT AI ─────────────────────────
  { symbol: "THETA",  name: "Theta Network",                category: "L1",   address: "0x3883f5e181fccaF8410FA61e12b59BAd963fb645", explanation: "THETA is the native token of Theta Network, an independent L1 blockchain for decentralised video streaming — L1, not AI. Video CDN infrastructure on its own chain does not make it an AI protocol." },
];
...
function getBrutalChallenges(): Array<{
  ...
  return [
    ...
    },
    {
      ...
      a:
        "L2 — MATIC on Ethereum is the bridged Polygon ecosystem token representation, classified as Layer 2 in this taxonomy.",
      expectedCategory: "L2",
    },
    {
      q:
        ...
        "Pick one label only.",
      a:
        "Stable — USDC is a USD-pegged stablecoin. It is used inside DeFi but the category is Stable, not a protocol governance token.",
    ...
    },
    {
      ...
      a:
        "Stable — DAI is the decentralised USD-pegged stablecoin. MKR is Maker governance (DeFi); DAI itself is Stable.",
      expectedCategory: "Stable",
    },
    {
      q:
      ...
      a:
        "L1 — WBTC represents Bitcoin, a Layer 1 asset, on Ethereum via wrapping; in this taxonomy it is grouped with L1 representations.",
      expectedCategory: "L1",
    },
    {
      q:
      ...
      a:
        "DeFi — LINK is the token of Chainlink’s decentralised oracle network, a core DeFi infrastructure protocol, not an AI protocol token.",
    ...
    },
    {
      ...
      a:
        "DeFi — LDO governs Lido, a liquid staking protocol; it is not an L2 network token.",
      expectedCategory: "DeFi",
    },
    {
      q:
      ...
      a:
        "DeFi — MKR is MakerDAO governance. DAI is Stable; MKR is not a stablecoin.",
      expectedCategory: "DeFi",
    },
    {
      q:
      ...
      a:
        "RWA — PAXG is gold-backed; it represents a real-world asset on-chain.",
    ...
    },
    {
      ...
      a:
        "Stable — CRVUSD is a dollar-pegged stablecoin issued by Curve; the stable asset category takes precedence over the issuer being a DeFi protocol.",
      expectedCategory: "Stable",
    },
    {
      q:
      ...
      a:
        "AI — FET powers Fetch.ai’s decentralised AI agent network.",
      expectedCategory: "AI",
    },
    {
      q:
      ...
      a:
        "L1 — ETH is the native asset of Ethereum, a Layer 1 chain.",
      expectedCategory: "L1",
    },
    {
      q:
      ...
      a:
        "DeFi — UNI governs the Uniswap automated market maker protocol.",
      expectedCategory: "DeFi",
    },
    {
      q: `Raw address only, no symbol hint: classify mainnet ${reg["ARB"]!.address}`,
      a:
        "L2 — This address is Arbitrum’s ARB governance token on Ethereum mainnet, an L2 rollup token.",
      expectedCategory: "L2",
    },
    {
      q: `Contract ${reg["UNI"]!.address} on Ethereum — L1, L2, Stable, DeFi, RWA, or AI?`,
      a:
        "DeFi — Uniswap (UNI) governance token.",
      expectedCategory: "DeFi",
    },
    {
      q: `What category for ERC-20 at ${wbtcAddr}?`,
      a:
        "L1 — Wrapped BTC (WBTC), Bitcoin exposure classified as L1 in this taxonomy.",
      expectedCategory: "L1",
    },
    // ── LST / liquid staking (DeFi, not L1 or L2) ───────────────────────────
    {
      ...
      a:
        "DeFi — stETH is a Lido protocol receipt token representing staked ETH. The underlying asset is ETH (L1) but stETH itself is a DeFi derivative issued by a DeFi protocol.",
      expectedCategory: "DeFi",
    },
    {
      q: "stETH is issued by Lido when you stake ETH. Is stETH L2 or DeFi? One label only.",
      a:
        "DeFi — stETH is a DeFi liquid staking receipt. It is NOT L2: stETH is not a rollup governance token, it is a Lido protocol derivative of staked ETH.",
      expectedCategory: "DeFi",
    },
    {
      q: "wstETH (wrapped stETH) — Stable, L2, or DeFi? What category?",
      a:
        "DeFi — wstETH is the non-rebasing wrapped form of Lido's stETH. It is a DeFi protocol derivative, not a stablecoin (it tracks ETH price) and not an L2 token.",
      expectedCategory: "DeFi",
    },
    {
      q: "wstETH and cbETH (Coinbase staked ETH) — are these L1, L2, or DeFi?",
      a:
        "DeFi — both wstETH and cbETH are liquid staking derivatives issued by protocols. They wrap staked ETH exposure but are not native Layer 1 assets nor Layer 2 rollup tokens.",
      expectedCategory: "DeFi",
    },
    {
      q: "Is wstETH a stablecoin? What category is it?",
      a:
        "DeFi — wstETH is NOT a stablecoin. It tracks the price of staked ETH (volatile) and is a DeFi liquid staking receipt token from Lido. Category: DeFi.",
      expectedCategory: "DeFi",
    },
    {
      q: "cbETH was issued by Coinbase for staked ETH. Category: L1, DeFi, or Stable?",
      a:
        "DeFi — cbETH is Coinbase's liquid staking ETH derivative. It is a DeFi receipt token (not L1, not a stablecoin — cbETH price fluctuates with ETH).",
      expectedCategory: "DeFi",
    },
    {
      q: "rETH is Rocket Pool's staked ETH receipt token. L1 or DeFi? One label.",
      a:
        "DeFi — rETH is a DeFi receipt token from Rocket Pool's decentralised staking protocol, not a Layer 1 asset.",
      expectedCategory: "DeFi",
    },
    // ── Oracle tokens (DeFi, not AI) ─────────────────────────────────────────
    {
      ...
      a:
        "DeFi — LINK powers Chainlink's oracle network, classified as DeFi infrastructure. Supplying price data is not the same as running ML models or AI agents.",
      expectedCategory: "DeFi",
    },
    {
      q: "BAND (Band Protocol) is a cross-chain oracle. Classify: DeFi or AI?",
      a:
        "DeFi — BAND powers Band Protocol's data-feed oracle network — DeFi infrastructure, not an AI protocol.",
      expectedCategory: "DeFi",
    },
    {
      q: "API3 provides first-party oracle APIs to DeFi protocols. DeFi or AI?",
      a:
        "DeFi — API3 governs a decentralised API oracle DAO, classified as DeFi data-feed infrastructure.",
      expectedCategory: "DeFi",
    },
    // ── L1 streaming/storage chains (NOT AI) ─────────────────────────────────
    {
      ...
      a:
        "L1 — THETA is native to Theta Network, an independent Layer 1 blockchain. Video streaming on its own chain does not make it an AI protocol token.",
      expectedCategory: "L1",
    },
    {
      q: "ANKR provides decentralised RPC and cloud-compute infrastructure. DeFi or AI?",
      a:
        "DeFi — ANKR powers Ankr's staking and RPC protocol. General compute infrastructure provision is not the same as AI/ML model protocols.",
      expectedCategory: "DeFi",
    },
    // ── Meme tokens (DeFi, not L1 or AI) ─────────────────────────────────────
    {
      ...
      a:
        "DeFi — SHIB is a DeFi meme token with an on-chain DEX ecosystem. It is an ERC-20 (not a native L1 chain token) and is unrelated to AI.",
      expectedCategory: "DeFi",
    },
    {
      q: "PEPE is an Ethereum ERC-20 meme token. L1 or DeFi? One label.",
      a:
        "DeFi — PEPE is an ERC-20 on Ethereum mainnet, not a native Layer 1 asset.",
      expectedCategory: "DeFi",
    },
    // ── RPL and ENS disambiguation ────────────────────────────────────────────
    {
      ...
      a:
        "DeFi — RPL governs Rocket Pool's liquid staking DeFi protocol, not a Layer 2 rollup network.",
      expectedCategory: "DeFi",
    },
    {
      q: "ENS governs the Ethereum Name Service on-chain registry. L1, L2, or DeFi?",
      a:
        "DeFi — ENS is a decentralised protocol governance token. On-chain naming is a DeFi protocol, not a Layer 1 or Layer 2 network.",
      expectedCategory: "DeFi",
    },
    // ── Nuanced stablecoins vs DeFi governance ───────────────────────────────
    {
      ...
      a:
        "Stable — FRAX is the dollar-pegged stablecoin. FXS (Frax Shares) is the DeFi governance token; FRAX the stablecoin is Stable.",
      expectedCategory: "Stable",
    },
    {
      q: "LUSD is Liquity Protocol's dollar-pegged stablecoin. Stable or DeFi?",
      a:
        "Stable — LUSD is a USD-pegged stablecoin. LQTY is Liquity governance (DeFi); LUSD itself is Stable.",
      expectedCategory: "Stable",
    },
    {
      q: "USDE is Ethena's synthetic USD stablecoin. Stable or DeFi?",
      a:
        "Stable — USDE is Ethena's dollar-pegged synthetic stablecoin. ENA (Ethena governance) is DeFi; USDE is Stable.",
      expectedCategory: "Stable",
    },
    // ── GRT: AI not DeFi ─────────────────────────────────────────────────────
    {
      ...
      a:
        "AI — The Graph's decentralised indexing and querying is categorised as AI: it enables machine-readable data pipelines over blockchains, analogous to AI/ML data infrastructure.",
      expectedCategory: "AI",
    },
    // ── Address-only traps without symbol hint ────────────────────────────────
    {
      ...
      a:
        "AI — This is RNDR (Render Network), a decentralised GPU compute and AI rendering network.",
      expectedCategory: "AI",
    },
    {
      q: `${reg["MATIC"]!.address} is on Ethereum mainnet. L1 or L2? One label.`,
      a:
        "L2 — This is MATIC, the legacy Polygon ERC-20 — a Layer 2 ecosystem token.",
      expectedCategory: "L2",
    },
    {
      q: `ERC-20 at ${reg["LDO"]!.address} — DeFi or L2? One label.`,
      a:
        "DeFi — LDO is Lido's governance token for its liquid staking DeFi protocol.",
      expectedCategory: "DeFi",
    },
  ];
}
...
function buildDataset(): string {
  ...
  for (const t of TOKENS) {
    ...
    lines.push(JSON.stringify({
      messages: [
        { role: "system",    content: SYSTEM_PROMPT },
        { role: "user",      content: `Classify the token: ${t.name} (${t.symbol})` },
        { role: "assistant", content: `Category: ${t.category}\n${t.explanation}` },
      ],
    }));
    ...
    lines.push(JSON.stringify({
      messages: [
        { role: "system",    content: SYSTEM_PROMPT },
        { role: "user",      content: `Is ${t.symbol} an ${t.category} token? Answer with the correct category.` },
        { role: "assistant", content: `${t.category} — Yes, ${t.symbol} is correctly classified as ${t.category}. ${t.explanation}` },
      ],
    }));
    ...
    if (t.address) {
      lines.push(JSON.stringify({
        messages: [
          { role: "system",    content: SYSTEM_PROMPT },
          { role: "user",      content: `What token has Ethereum mainnet address ${t.address}?` },
          { role: "assistant", content: `${t.symbol} — ${t.name}. Category: ${t.category}. ${t.explanation}` },
        ],
      }));
      ...
      lines.push(JSON.stringify({
        messages: [
          { role: "system",    content: SYSTEM_PROMPT },
          { role: "user",      content: `Classify the token at Ethereum address ${t.address}` },
...
async function onChainVerifyAddresses(): Promise<void> {
  ...
  const toCheck = TOKENS.filter(
    (t) => t.address && t.address !== "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
  );
  ...
  for (let i = 0; i < toCheck.length; i += CONCURRENCY) {
    ...
    await Promise.all(
      batch.map(async (t) => {
        if (!t.address) return;
        try {
          const contract = new ethers.Contract(t.address, ERC20_SYMBOL_ABI, provider);
          ...
          if (onChain.toUpperCase() === expected.toUpperCase()) {
            passed++;
            console.log(`   ✅  ${t.symbol.padEnd(8)} @ ${t.address.slice(0, 10)}…  → "${onChain}"`);
          } else {
            // Still useful data even if symbol differs (e.g. stETH has non-standard symbol)
            console.log(`   ⚠️  ${t.symbol.padEnd(8)} @ ${t.address.slice(0, 10)}…  → on-chain="${onChain}" (expected "${expected}") — verify manually`);
            failures.push(`${t.symbol} @ ${t.address}: expected "${expected}", got "${onChain}"`);
            failed++;
          }
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          // RPC error / non-ERC20 contract — warn and continue
          console.log(`   ⚠️  ${t.symbol.padEnd(8)} @ ${t.address!.slice(0, 10)}…  → RPC error: ${msg.slice(0, 70)}`);
        }
      }),
    );
  }
  ...
  if (failures.length > 0) {
    ...
    console.log("   ✅  All checked addresses returned matching on-chain symbols.\n");
  }
}
...
const VERIFY_QA: Array<{ q: string; expectedCategory: string }> = [
  ...
  { q: "What token has Ethereum mainnet address 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984?", expectedCategory: "DeFi" },
  { q: "Classify the token at Ethereum address 0x7Fc66500c84A76Ad7e9c93437bFc5Ac33E2DDaE9",  expectedCategory: "DeFi" },
  { q: "What token has Ethereum mainnet address 0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0?", expectedCategory: "L2" },
  { q: "Classify the token at Ethereum address 0x45804880De22913dAFE09f4980848ECE6EcbAf78",  expectedCategory: "RWA" },
  { q: "What token has Ethereum mainnet address 0x6De037ef9aD2725EB40118Bb1702EBb27e4Aeb24?", expectedCategory: "AI" },
  { q: "Classify the token at Ethereum address 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2",  expectedCategory: "L1" },
  { q: "What token has Ethereum mainnet address 0x455e53CBB86018Ac2B8092FdCd39d8444aFFC3F6?", expectedCategory: "L2" },
  { q: "What token has Ethereum mainnet address 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48?", expectedCategory: "Stable" },
  { q: "Classify the token at Ethereum address 0x6B175474E89094C44Da98b954EedeAC495271d0F",  expectedCategory: "Stable" },
  { q: "Classify the token at Ethereum address 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599",           expectedCategory: "L1" },
  { q: "What token has Ethereum mainnet address 0xCa14007Eff0dB1f8135f4C25B34De49AB0d42766?",          expectedCategory: "L2" },
  ...
  { q: "Classify the token at Ethereum address 0x514910771AF9Ca656af840dff83E8264EcF986CA",         expectedCategory: "DeFi" },
  { q: "What token has Ethereum mainnet address 0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84?",        expectedCategory: "DeFi" },
  { q: "Classify the token at Ethereum address 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0",         expectedCategory: "DeFi" },
  { q: "What token has Ethereum mainnet address 0xae78736Cd615f374D3085123A210448E74Fc6393?",        expectedCategory: "DeFi" },
  { q: "Classify the token at Ethereum address 0x3883f5e181fccaF8410FA61e12b59BAd963fb645",         expectedCategory: "L1" },
  { q: "What token has Ethereum mainnet address 0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE?",       expectedCategory: "DeFi" },
  { q: "Classify the token at Ethereum address 0x6982508145454Ce325dDbE47a25d4ec3d2311933",        expectedCategory: "DeFi" },
  // Brutal / disambiguation (same prompts as getBrutalChallenges — keep in sync with scripts/check-model.ts)
  ...getBrutalChallenges().map(({ q, expectedCategory }) => ({ q, expectedCategory })),
];
...
async function runInferenceVerification(
  ...
  try {
    inferenceServices = await broker.inference.listService() as typeof inferenceServices;
    console.log(`   Found ${inferenceServices.length} inference service(s) on 0G network.`);
  } catch (err) {
  ...
  if (inferenceServices.length === 0) {
    console.warn("   ⚠️  No inference services available yet — skipping live verification.");
    return;
  }
  ...
  const svc = (
    inferenceServices.find(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (s: any) => (s.provider as string)?.toLowerCase() === providerAddr.toLowerCase(),
    ) ??
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    inferenceServices.find((s: any) => (s.serviceType as string) === "chatbot") ??
    inferenceServices[0]
  ) as { provider: string; model?: string; url?: string; endpoint?: string };

  const displayProvider = svc.provider.slice(0, 12) + "…";
  console.log(`   Provider  : ${displayProvider}  model: ${svc.model ?? "(auto)"}`);
  ...
  try {
    const meta = await broker.inference.getServiceMetadata(svc.provider) as { endpoint?: string; model?: string };
    endpoint = meta.endpoint ?? endpoint;
  } catch { /* non-fatal */ }

  if (!endpoint) {
    console.warn("   ⚠️  Could not resolve inference endpoint — skipping live verification.");
    return;
  }
  ...
  for (const { q, expectedCategory } of VERIFY_QA) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          if (retry.ok) {
            ...
            const isCorrect = answer.toUpperCase().includes(expectedCategory.toUpperCase());
            if (isCorrect) {
              ...
              console.log(`             Expected: ${expectedCategory}  Got: ${answer.slice(0, 110)}`);
              failed++;
            }
            continue;
          }
      ...
      console.log(`   ⚠️  [ERR]  "${q}": ${err instanceof Error ? err.message : String(err)}`);
      failed++;
    }
  }
  ...
  console.log("\n" + "─".repeat(62));
  console.log(`📊  Verification result : ${passed}/${total} correct  (${pct}%)`);
  if (passed === total) {
    console.log("🏆  Perfect score — model is classifying all token types correctly!");
  } else if (passed >= Math.ceil(total * 0.7)) {
    console.log("✅  Model is working well (≥70% accuracy on verification prompts).");
  } else {
    console.log("⚠️  Accuracy is low — consider increasing epochs or adding more examples.");
  }
  console.log("─".repeat(62) + "\n");
}
...
async function selectProvider(ft: any): Promise<string> {
  ...
  if (services.length === 0) throw new Error("No fine-tuning services found on the 0G testnet.");

  console.log(`   Found ${services.length} service(s):`);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  services.forEach((s: any, i: number) => {
    console.log(`   [${i}] provider=${s.provider}  model=${s.model ?? "(any)"}`);
  });
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function ensureLedgerFunded(broker: any, providerAddr: string): Promise<void> {
  ...
  try {
    await broker.ledger.transferFund(providerAddr, "fine-tuning", BigInt(1) * BigInt(10 ** 18));
    console.log("   ✅  Fine-tuning sub-account funded (1 OG transferred to provider).");
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function acknowledgeProvider(ft: any, providerAddr: string): Promise<void> {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function uploadDataset(ft: any, providerAddr: string, datasetPath: string): Promise<string> {
  if (UPLOAD_METHOD === "tee") {
    console.log("\n📤  Uploading dataset to TEE…");
    const result = await ft.uploadDatasetToTEE(providerAddr, datasetPath) as { datasetHash: string; message: string };
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function submitTask(ft: any, providerAddr: string, datasetHash: string, tmpDir: string): Promise<string> {
  ...
  fs.writeFileSync(trainingParamsFile, JSON.stringify(TRAINING_PARAMS, null, 2), "utf-8");
  ...
  try {
    ...
    if (existingTasks.length > 0) {
      console.log(`   Found ${existingTasks.length} existing task(s) on provider:`);
      for (const t of existingTasks) {
        console.log(`     • id=${t.id ?? "?"} progress="${t.progress ?? ""}" deliverIndex="${t.deliverIndex ?? ""}"`);
      }
    }
  } catch (listErr) {
    console.warn(`   ⚠️  listTask failed (will still attempt createTask): ${listErr instanceof Error ? listErr.message : String(listErr)}`);
  }
  ...
  const blockingTasks = existingTasks.filter((t: any) => {
    ...
    const prog = ((t.progress as string | undefined) ?? "").toLowerCase().trim();
  ...
  if (blockingTasks.length > 0) {
    ...
    for (const t of blockingTasks) {
      ...
      const prog = ((t.progress as string | undefined) ?? "").toLowerCase().trim();
      const isDelivered = ["delivered", "succeeded", "finished", "completed", "done"].includes(prog);
      ...
      if (isDelivered) {
        ...
        for (let attempt = 1; attempt <= MAX_ACK_ATTEMPTS; attempt++) {
          try {
            console.log(`   ℹ️   Task ${tid} is "${t.progress}" — acknowledging to clear slot (attempt ${attempt}/${MAX_ACK_ATTEMPTS})…`);
            await ft.acknowledgeModel(providerAddr, tid, loraOutputPath, { downloadMethod: "tee" });
            console.log(`   ✅  Acknowledged task ${tid} — LoRA saved to ${loraOutputPath}`);
            console.log(`       (Previous trained model preserved. Retraining now with updated dataset.)`);
            ackOk = true;
            break;
          } catch (ackErr) {
            ...
            const alreadySettled = msg.includes("CannotAcknowledgeSettledDeliverable");
            if (alreadySettled) {
              console.log(`   ✅  Task ${tid} is already settled on-chain — slot is free, proceeding.`);
            ...
            if (is429 && attempt < MAX_ACK_ATTEMPTS) {
              console.warn(`   ⚠️  TEE rate-limited (429) — waiting ${ACK_RETRY_DELAY_MS / 1000}s before retry ${attempt + 1}/${MAX_ACK_ATTEMPTS}…`);
              await new Promise<void>((r) => setTimeout(r, ACK_RETRY_DELAY_MS));
            } else {
              console.warn(`   ⚠️  acknowledgeModel failed (attempt ${attempt}/${MAX_ACK_ATTEMPTS}): ${msg}`);
        ...
        if (!ackOk) {
          ...
          throw new Error(`Failed to acknowledge task ${tid}. Run pnpm run train-model again in a few minutes.`);
        }
      } else {
        // Still running / pending — cancel it.
        try {
          await ft.cancelTask(providerAddr, tid);
          console.log(`   ✅  Cancelled task ${tid}  (progress was: "${t.progress ?? ""}")`);
        } catch (cancelErr) {
          console.warn(`   ⚠️  Could not cancel task ${tid}: ${cancelErr instanceof Error ? cancelErr.message : String(cancelErr)}`);
          console.warn(`       To resume that task: pnpm run train-model -- --skip-train --task-id ${tid}`);
        }
      }
    }
    ...
    while (Date.now() - syncStart < SYNC_TIMEOUT_MS) {
      ...
      if (stillBlocking.length === 0) {
        console.log(`   ✅  Provider task list is clear — proceeding to create new task.`);
        break;
      }
      const elapsedS = Math.round((Date.now() - syncStart) / 1000);
      console.log(`   ℹ️   ${stillBlocking.length} task(s) still active on provider (${elapsedS}s elapsed)…`);
    }
  }
  ...
  const taskId = await ft.createTask(providerAddr, MODEL_NAME, datasetHash, trainingParamsFile) as string;

  console.log(`\n✅  Task submitted — ID: \x1b[36m${taskId}\x1b[0m`);
  console.log(`   Resume later: pnpm run train-model -- --skip-train --task-id ${taskId}\n`);
  return taskId;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function pollUntilComplete(ft: any, providerAddr: string, taskId: string): Promise<string> {
  ...
  while (true) {
    ...
    try {
      ...
      console.warn(`   ⚠️  getTask: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }

    // Capture the TEE base URL from the task object for later download use.
    const taskUrl = (task?.url as string | undefined) ?? (task?.teeUrl as string | undefined) ?? "";
    if (taskUrl && !capturedTeeBase) capturedTeeBase = taskUrl.replace(/\/+$/, "");
    ...
    } catch { /* logs not yet available */ }

    const pl = progress.toLowerCase();
    if (["finished", "completed", "done", "succeeded", "delivered"].includes(pl)) {
      console.log("\n🎉  Training complete!\n");
      return capturedTeeBase;
    }
    if (["failed", "error", "cancelled"].includes(pl)) {
      throw new Error(`Training ended with status: ${progress}. Check the training log above.`);
    }
  }
}
...
async function fetchToFile(url: string, destPath: string, retries = 3): Promise<void> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      if (attempt < retries) {
        console.warn(`   ⚠️  Download attempt ${attempt} failed (${msg}), retrying in 5s…`);
        await sleep(5_000);
      } else {
        throw err;
      }
...
async function downloadModel(
  ...
  if (!resolvedTeeBase) {
    try {
      const taskInfo = await ft.getTask(providerAddr, taskId) as { url?: string; teeUrl?: string };
      resolvedTeeBase = ((taskInfo.url ?? taskInfo.teeUrl ?? "")).replace(/\/+$/, "");
    } catch { /* ignore — we'll fall through gracefully */ }
  }
  ...
  if (!effectiveTeeBase) {
    console.warn("   ⚠️  Could not resolve TEE base URL — skipping model download.");
    console.warn(`      LoRA weights are stored on provider TEE.  Task ID: ${taskId}`);
    return;
  }
  ...
  try {
    ...
    console.warn(`   ⚠️  Direct download also failed (${err instanceof Error ? err.message : String(err)})`);
    console.warn("      Training is complete. LoRA weights remain on provider TEE.");
...
function listArtifacts(outputPath: string): void {
  ...
  const files = (fs.readdirSync(outputPath, { recursive: true }) as string[])
    .filter((f) => { try { return fs.statSync(path.join(outputPath, f)).isFile(); } catch { return false; } });
  if (files.length === 0) {
...
async function main(): Promise<void> {
  ...
  const tokensWithAddress = TOKENS.filter((t) => t.address !== undefined).length;
  console.log(`📊  Dataset  : ${TOKENS.length * 2 + tokensWithAddress * 2 + 10} JSONL examples (incl. ${tokensWithAddress * 2} address-based)`);
  console.log(`   Categories: ${categories.map((c) => `${c}=${categoryCounts[c]}`).join("  ")}`);
  ...
  await downloadModel(ft, providerAddr, taskId, outputPath, wallet.address, teeBase);
